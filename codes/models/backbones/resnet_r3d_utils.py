...
building the model and for loading model parameters.
These helper functions are built to mirror
those in the official TensorFlow implementation.
"""
...
def add_conv3d(in_filters, out_filters, kernel,
               stride, pad, block_type='3d',
               group=1, with_bn=True):
    """add conv3d"""
    if with_bn:
    ...
    if block_type == '2.5d':
        ...
        conv_s = conv3d(in_filters, middle_filters,
                        kernel=[1, kernel[1], kernel[2]],
                        stride=[1, stride[1], stride[2]],
                        pad=[0, pad[1], pad[2]])
        bn_s = nn.BatchNorm3d(middle_filters, eps=1e-3)
        conv_t = conv3d(middle_filters, out_filters,
                        kernel=[kernel[0], 1, 1],
                        stride=[stride[0], 1, 1],
                        pad=[pad[0], 0, 0])
        if with_bn:
            return ModuleList(
                [conv_s, bn_s, nn.ReLU(), conv_t],
                ['conv_s', 'bn_s', 'relu_s', 'conv_t'])
        else:
            return ModuleList(
                [conv_s, nn.ReLU(), conv_t],
                ['conv_s', 'relu_s', 'conv_t'])
    if block_type == '0.3d':
        conv_T = conv3d(in_filters, out_filters,
                        kernel=[1, 1, 1],
                        stride=[1, 1, 1],
                        pad=[0, 0, 0])
        ...
        if with_bn:
            return ModuleList(
                [conv_T, bn_T, nn.ReLU(), conv_C],
                ['conv_T', 'bn_T', 'relu_T', 'conv_C'])
        else:
            return ModuleList(
                [conv_T, nn.ReLU(), conv_C],
                ['conv_T', 'relu_T', 'conv_C'])
    if block_type == '3d':
